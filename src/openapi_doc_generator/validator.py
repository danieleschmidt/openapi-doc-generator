    ...
    def _validate_paths_section(self, spec: Dict[str, Any]) -> List[str]:
        ...
        for path, operations in paths.items():
            if not isinstance(operations, dict):
                suggestions.append(f"Path '{path}' must contain an object with operations")
                continue
            ...
            for method, operation in operations.items():
                ...
                if method.lower() not in self.VALID_HTTP_METHODS:
                    suggestions.append(f"Invalid HTTP method '{method}' in path '{path}'")
                    continue
                ...
                if "summary" not in operation:
                    suggestions.append(f"Operation '{method} {path}' is missing summary")
                
                # Check for missing responses
                if "responses" not in operation:
                    suggestions.append(f"Operation '{method} {path}' is missing responses")
                elif not isinstance(operation["responses"], dict):
                    suggestions.append(f"Operation '{method} {path}' responses must be an object")
                elif not operation["responses"]:
                    suggestions.append(f"Operation '{method} {path}' has no response definitions")
    ...
    def _validate_components_section(self, spec: Dict[str, Any]) -> List[str]:
        ...
        if schemas and isinstance(schemas, dict):
            for schema_name, schema_def in schemas.items():
                ...
                if not schema_def:
                    suggestions.append(f"Schema '{schema_name}' is empty - consider adding 'type' or 'properties'")
                elif "type" not in schema_def and "properties" not in schema_def and "$ref" not in schema_def:
                    suggestions.append(f"Schema '{schema_name}' should define 'type', 'properties', or '$ref'")
    ...
    def _validate_security(self, spec: Dict[str, Any]) -> List[str]:
        ...
        for scheme in referenced_schemes:
            if scheme not in security_schemes:
                suggestions.append(f"Security scheme '{scheme}' is referenced but not defined in components.securitySchemes")
